    ...
    <section>
      <Form ref="formCustom" :model="addQuestion" :rules="ruleValidate" :label-width="80">
        ...
        <div style="width:60%;margin-left: 80px;">
          <upload-model :upData="upData"></upload-model>
        </div>
        <FormItem>
          <Button class="mt20" type="primary" @click="handleSubmit('formCustom')">提交</Button>
          <Button class="mt20" @click="goBack">返回</Button>
        </FormItem>
      </Form>
    </section>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit (name) {
        let url = this.url + 'Document/WIKI.Create'
//        let content = this.$refs.ue.getUEContent()
//        this.addQuestion.Answers.push(content)
        this.addQuestion.Tags = this.tags
        this.addQuestion.DocumentAttachments = this.getCommon.fileCenter
        let _this = this
        this.$refs[name].validate((valid) => {
          if (valid) {
            o(url).post({dto: this.addQuestion}).save().then(function (data) {
              _this.upData.Data = `{'Id':'null','EventType':'Document'}`
              _this.upShow = true
