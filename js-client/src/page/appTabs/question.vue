...
<script>
  import problemModel from '../../components/problemModel/problemModel.vue'
  import o from 'o.js'
  ...
  export default {
    ...
    watch: {
      ...
      'getStore.searchBoolean': {
        handler: function (val, oldVal) {
          if (this.getStore.appTabs === 'question') {
            this.pageCurrent = 1
            this.searchFn()
          }
        },
...
//        设置最后条数显示
        ...
        let url = this.url + `Content/WIKI.SearchQuestion?$filter=ContentType
        eq 'Question' and (contains(Title,'${keyword}'))&$top=${pageSize}&$skip=${skip}&$count=true`
        // todo 最后一页会重复
...
//            _this.$Message.warning('查无相关数据')
          ...
          qData.forEach(function (item) {
            if (item.Question.CreatedTime !== null) {
              item.Question.CreatedTime = item.Question.CreatedTime.split('T')[0]
            }
            if (item.Question.UpdatedTime !== null) {
              item.Question.UpdatedTime = item.Question.UpdatedTime.split('T')[0]
            }
          })
