<template>
  <div>
    ...
    >
      <slot>
        <Button type="primary" @click="routerChange('addProblem')">
          <Icon type="plus"></Icon>
        </Button>
      </slot>
    </xheaderBar>
...
<script>
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        options: Object.assign({}, this.$xvuex.options, {   // options将存入vuex,基础配置看 cVuex.options
          ...
          title: '我的问题',  // 本页面名称
          gridKey: 'QuestionView',  // 本页面 Eng名，唯一
          arr: [
            {
              type: 'selection',
              width: 60,
                ...
                return h('div',
                  {
                    ...
                    (function () {
                      if (params.row.Answer_Text !== null) {
                        return h(
                          'Button', {
                            domProps: {
                              innerHTML: '已回复'
    ...
    methods: {
      filterIsReply (value, row) {
        let _self = this
        let filter = null
        if (value.includes(true)) {
          filter = `Answer_Text ne null`
        } else {
          filter = `Answer_Text eq null`
        }
        let url = _self.$baseUrl.wiki + `QuestionView?$filter=${filter}`
        _self.$store.dispatch(_self.options.gridKey + '_set_state_data', {url: url})
//        o(urlAppend(url, {r: Math.random()})).get(function (data) {
//          _self.$store.dispatch(_self.options.gridKey + '_set_state_data', {tableData: data})
//          console.log(data)
//        })
      },
      ...
      delData: function (item) {
        ...
        _this.$Modal.confirm({
          ...
          onOk: function () {
            let url = _this.url + `Question(${item.Id})`
            ...
            _self.$Modal.confirm({
              ...
              onOk: function () {
                delObjs.forEach(function (Item) {
                  let url = _self.$baseUrl.wiki + `Question(${Item.Id})`
