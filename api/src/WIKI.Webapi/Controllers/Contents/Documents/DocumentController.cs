    ...
    public class DocumentController : ODataController
    {
        ...
        public IHttpActionResult Update([FromODataUri] long key, ODataActionParameters parameters)
        {
            ...
            using (var transaction = Db.Database.BeginTransaction())
            {
                ...
                foreach (var item in documentEntity.DocumentAttachmentList.ToList())
                {
                    Db.DocumentAttachment.Attach(item);
                ...
                {
                    var attachmentEntity = item.MapToEntity();
                    attachmentEntity.DocumentId = documentEntity.Id;
                    attachmentEntity.CreatedBy = User.Identity.Name;

                    Db.DocumentAttachment.Add(attachmentEntity);
                }
